...
export function hydrate(item: HTMLElement) {
  ...
  return import(`../browser-components/${modfedComponent}`)
    .then((mod) => {
      ...
      if (mod.customElements) {
        console.log("loading scripts for customElements", mod.customElements);
        return Promise.all(mod.customElements.map((elem) => import(`../web-components/${elem}`)));
      }
    })
    .catch((err) => {
      console.error(err);
