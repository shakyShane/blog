...
export function H1(props: PropsWithChildren<any>) {
  return (
    <h1 className="font-bold font-sans break-normal text-gray-900 pt-6 pb-2 text-3xl md:text-4xl">{props.children}</h1>
  );
}

export function H2(props: PropsWithChildren<any>) {
  return (
    <h2 className="font-bold font-sans break-normal text-gray-900 pt-6 pb-2 text-2xl md:text-2xl">{props.children}</h2>
  );
}

export function H3(props: PropsWithChildren<any>) {
  return (
    <h3 className="font-bold font-sans break-normal text-gray-900 pt-4 pb-2 text-1xl md:text-1xl">{props.children}</h3>
  );
}
...
export function HomeLink() {
  return (
    <p className="font-sans text-base mb-4 md:text-sm text-green-500 font-bold">
      <Link href={"/"}>
        <a className="text-base md:text-sm text-green-500 font-bold no-underline hover:underline">&lt; BACK HOME</a>
      </Link>
    </p>
  );
}
