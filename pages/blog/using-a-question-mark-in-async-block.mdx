...
Putting the conversation about whether this is an optimal way to read a file's byte length to one side,
let's take a look.

`std::fs::read_to_string("Cargo.toml")` returns `Result<String, io::Error>` so
we tried to use the question mark operator `?` to 'unwrap' the `String` that we really care about.
...
why's it mentioning the return types at all?
...
🥰 ahh, this kind of example is one of the reasons I enjoy Rust so much 🦀
...
Because of this, we've been able to write a couple of different solutions that that didn't require much
...
Back to basics for a second here - in the examples above, we've taken the requirement of "read 2 files from disk and sum their
...
- 1) + 2) notice how on both of these lines, we add a `?` directly after the `read_to_string()` call. This will either 'unwrap' the
value (if it was successful), or it will return early with the error. Yep - exactly what we manually implemented earlier can be achieved
with that simple character.
...
`?` operator to work it's only the `Err` part that needs to line up.
...
```rust {"attentionLines": [0]}
fn main() {
    let toml = std::fs::read_to_string("Cargo.toml")?;
    println!("Cargo.toml has {} bytes", toml.len());
}
```
...
error[E0308]: mismatched types
 --> src/async_block_01.rs:5:16
  |
5 |         return 8;
  |                ^ expected `()`, found integer
```

<Highlight src={require("raw-loader!../../src/async_block_01.rs").default}
           lang={"rust"}
           lines={[4, 5, 6, 7, 8]}
           attentionLines={[4]}
/>

<Highlight src={require("raw-loader!../../errors/async_block_01.txt").default}
    attentionLines={[6]}
/>
...
export default ({ children }) => (
  <Layout
    meta={{
      title: "Unwrapping values with the question mark operator in Rust",
    }}
  >
    {children}
  </Layout>
